...
//下拉菜单:name(点击的标签名称),name1(下拉的具体标签名称)
//str(需要改变的高度:{'height':10px})
function select(name,name1,str){
	for( let i=0;i<name.length;i++ ){
		// name[i].index=i;
		name[i].istrue=true;
		name[i].onclick=function(){
			if( this.istrue ){
				// name1[this.index].style.display='block';
				name1[i].style.display='block';
				// startMove(name1[this.index],str);
				startMove(name1[i],str);
			}else{
				startMove(name1[i],{'height':0});
				// startMove(name1[this.index],{'height':0});
			}
...
//把新节点插入到目标节点的最后(newElement:新插入节点的名称,targetElement:目标节点的名称)
...
//抛物线运动封装
//elem:待操作的html元素
//targetX:目标元素的offsetLeft
//targetY:目标元素的offsetTop
